...
<iframe width="560" height="315" src="https://www.youtube.com/embed/KXNIx4sfzns?si=fzW2P-jG3r8xTAds&amp;controls=0&amp;start=80" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
ggsave("assets/couch_colours_waffle_chart.png", width = 8, height = 8, dpi = 300)
```
...
This part was relatively straightforward. Using the `yt_dlp` library, I pulled video information from the official NeverTooSmall playlist and stored the thumbnails in a folder in my repo. With this, I could access the necessary data on each video right in my environment.
...
```{r}
make_tab <- function(colour) {         # function to make the tabs
  cat("::: {.panel}\n")             # Open tab
  cat("##", colour, "{.panel-name}\n") # Label tab
  p <- make_couch_table(colour)           # Create plot
...
colour_list <- df_table_in %>%
    count(couch_colour, sort = T) %>%
    filter(n > 3) %>%
    pull(couch_colour)


purrr::walk(colour_list, make_tab)     # make the tabs for each island that is there
```
